...
exports.patcher = (fs = fs$1, root, guards) => {
    ...
    const { isEscape, isGuardPath, isOutPath } = exports.escapeFunction(root, guards);
    // tslint:disable-next-line:no-any
    fs.lstat = (...args) => {
        ...
        if (cb) {
            ...
            args[args.length - 1] = (err, stats) => {
                ...
                // if this is not a symlink or the path is not inside the root and not in a guard it has
                // no way to escape.
                if (!stats.isSymbolicLink() || (isOutPath(linkPath) && !isGuardPath(linkPath))) {
