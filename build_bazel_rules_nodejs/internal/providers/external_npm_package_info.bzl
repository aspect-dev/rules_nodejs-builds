...
ExternalNpmPackageInfo = provider(
    ...
    fields = {
        ...
        "directory_artifacts": "True if any sources are directory artifacts",
...
def _node_modules_aspect_impl(target, ctx):
    ...
    # if any deps have directory_artifacts then directory_artifacts will be true
    directory_artifacts = False

    if hasattr(ctx.rule.attr, "deps"):
        for dep in ctx.rule.attr.deps:
            if ExternalNpmPackageInfo in dep:
                directory_artifacts = directory_artifacts or dep[ExternalNpmPackageInfo].directory_artifacts
