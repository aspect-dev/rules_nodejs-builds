...
""",
    ...
    "experimental_directory_artifacts": attr.bool(
        default = False,
        doc = """Expose only top level package directory artifacts from node_modules.
        
Requires using directory_file_path rule and/or DirectoryFilePathInfo provider to reference
files within npm packages.""",
    ),
...
NB: This option has no effect when experimental_directory_artifacts is True as all files are
...
def _create_build_files(repository_ctx, rule_type, node, lock_file, generate_local_modules_build_files):
    ...
    generate_config_json = struct(
        experimental_directory_artifacts = repository_ctx.attr.experimental_directory_artifacts,
