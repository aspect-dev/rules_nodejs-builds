...
let config = {
    experimental_directory_artifacts: false,
...
function main() {
    ...
    if (config.experimental_directory_artifacts) {
        NODE_MODULES_PACKAGE_NAME = '$node_modules_da$';
    }
...
function generatePackageBuildFiles(pkg) {
    ...
    let buildFile = config.experimental_directory_artifacts ?
...
`;
        ...
        for (const [name, path] of executables.entries()) {
            const entryPoint = config.experimental_directory_artifacts ?
